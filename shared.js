  ...
  document.getElementById('vp-body').innerHTML = `
    ...
    <div class="vp-body-wrap">
      <div class="vp-body-inner">
        ...
        ${isPremiumActive(v) && v.rooms && v.rooms.length > 0 ? '<div class="vp-section fade-up"><div class="vp-eyebrow">Konaklama</div><h2 class="vp-stitle">Oda Tipleri</h2><div style="display:grid;gap:12px;">' + '<div class="vp-rooms-grid" style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;">' + v.rooms.map(function(r) { var viewIcons = {"Deniz Manzarası":"🌊","Kara Manzarası":"🏔","Bahçe Manzarası":"🌿","Havuz Manzarası":"🏊","Deniz & Kara Manzarası":"🌊🏔"}; return '<div style="position:relative;border-radius:18px;overflow:hidden;transition:all .35s cubic-bezier(.16,1,.3,1);cursor:default;" onmouseover="this.style.transform=\'translateY(-4px)\';this.style.boxShadow=\'0 16px 40px rgba(26,39,68,.12)\'" onmouseout="this.style.transform=\'\';this.style.boxShadow=\'\'">' + (r.image ? '<div style="aspect-ratio:16/10;overflow:hidden;background:linear-gradient(90deg,#e8e4de 25%,#f0ece6 50%,#e8e4de 75%);background-size:800px 100%;"><img src="' + r.image + '" style="width:100%;height:100%;object-fit:cover;opacity:0;transition:opacity .6s ease;" loading="lazy" onload="this.style.opacity=1" alt="' + r.name + '"></div>' : '<div style="aspect-ratio:16/10;background:linear-gradient(160deg,#1A2744,#2A3A5A);display:flex;align-items:center;justify-content:center;font-size:2.5rem;">🛏</div>') + '<div style="position:absolute;bottom:0;left:0;right:0;padding:10px 10px 12px;background:linear-gradient(to top,rgba(10,18,36,.88) 0%,rgba(10,18,36,.5) 50%,transparent 100%);">' + '<div style="font-weight:700;font-size:.88rem;color:#F5EDE0;margin-bottom:6px;text-shadow:0 1px 3px rgba(0,0,0,.3);">' + r.name + '</div>' + '<div style="display:flex;flex-wrap:wrap;gap:4px;">' + [r.capacity ? '<span style="font-size:.6rem;font-weight:600;padding:3px 8px;border-radius:6px;background:rgba(255,255,255,.12);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);color:rgba(245,237,224,.85);">👤 ' + r.capacity + ' Kişi</span>' : '', r.bed ? '<span style="font-size:.6rem;font-weight:600;padding:3px 8px;border-radius:6px;background:rgba(255,255,255,.12);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);color:rgba(245,237,224,.85);">🛏 ' + r.bed + '</span>' : '', r.size ? '<span style="font-size:.6rem;font-weight:600;padding:3px 8px;border-radius:6px;background:rgba(255,255,255,.12);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);color:rgba(245,237,224,.85);">📐 ' + r.size + ' m²</span>' : '', r.view ? '<span style="font-size:.6rem;font-weight:600;padding:3px 8px;border-radius:6px;background:rgba(26,107,138,.3);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);color:#7DD3FC;">' + (viewIcons[r.view] || '🏞') + ' ' + r.view + '</span>' : ''].filter(Boolean).join('') + '</div></div></div>'; }).join('') + '</div>' + '</div></div>' : ''}
