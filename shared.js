...
// Reel paylaş — native share API + fallback clipboard
window.akReelShare = function(url, title) {
  try {
    if (navigator.share) {
      navigator.share({ title: (title || 'Assos') + ' — Assos\'u Keşfet', url: url }).catch(function(){});
    } else if (navigator.clipboard) {
      navigator.clipboard.writeText(url).then(function() {
        if (typeof showToast === 'function') showToast('Link kopyalandı', 'success');
        else alert('Link kopyalandı: ' + url);
      });
    } else {
...
// Reel kaydetme — localStorage "bookmark" toggle
window.akReelBookmark = function(btn, reelKey) {
  try {
    var key = 'reel_bookmark_' + reelKey;
    var saved = localStorage.getItem(key) === '1';
    var icon = btn.querySelector('.ak-reel-bookmark');
    if (!saved) {
      localStorage.setItem(key, '1');
      if (icon) icon.setAttribute('fill', '#262626');
    } else {
      localStorage.removeItem(key);
      if (icon) icon.setAttribute('fill', 'none');
    }
    btn.animate([{ transform:'scale(1)' }, { transform:'scale(1.2)' }, { transform:'scale(1)' }], { duration:250 });
  } catch(e) {}
};

// Reel state restore — sayfa yuklenince like/bookmark durumlari
window.akReelRestoreState = function() {
  document.querySelectorAll('[id^="reel-"]').forEach(function(card) {
    try {
      var key = card.id;
      // Like
      ...
      if (localStorage.getItem('reel_bookmark_' + key) === '1') {
        var bm = card.querySelector('.ak-reel-bookmark');
        if (bm) bm.setAttribute('fill', '#262626');
      }
    } catch(e) {}
  });
};
...
window.akBuildReelsHtml = function(reels, opts) {
  ...
  reels.forEach(function(reel, idx) {
    ...
    // IG CTA
    html += '<a href="' + _escAttr(reel.url) + '" target="_blank" rel="noopener noreferrer" style="display:flex;align-items:center;gap:6px;padding:10px 14px 14px;font-size:.76rem;font-weight:600;color:#00376B;text-decoration:none;">';
...
window.akPlayReel = function(cardId, srcUrl, type) {
  ...
  if (igOpen) igOpen.style.display = '';
};
...
(function injectStyles() {
  ...
  style.textContent = `
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    /* Image error fallback — kirilan gorselde shimmer durmasin, emoji fallback gorunsun */
...
function renderNav(opts = {}) {
  ...
  const navHTML = `
    <div id="mobile-menu" role="dialog" aria-label="Navigasyon menüsü" aria-hidden="true">
      ...
      <div class="mm-links">
        ${links.map((l, i) => {
          if (l.dropdown) {
            // Mobile: her zaman açık, static başlık + alt menü görünür
            var groupActive = isActive(l.href);
            return `
              <div class="mm-group expanded${groupActive ? ' active' : ''}">
                <div class="mm-link mm-group-title">
  ...
  if (!document.getElementById('save-drawer-styles')) {
    ...
    s.textContent = `
      ...
      .sd-empty-actions{display:flex;flex-direction:column;gap:8px;width:100%;max-width:240px;margin:4px auto 0;}
      .sd-empty-btn{display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:9px 18px;border-radius:999px;font-size:.78rem;font-weight:600;text-decoration:none;transition:transform .15s,box-shadow .2s;box-shadow:0 1px 3px rgba(0,0,0,.06);}
      .sd-empty-btn:hover{transform:translateY(-1px);box-shadow:0 4px 10px rgba(0,0,0,.08);}
      .sd-empty-btn-venue{background:#1A2744;color:#fff;}
      .sd-empty-btn-venue:hover{background:#2A3A5A;}
      .sd-empty-btn-place{background:#1A6B8A;color:#fff;}
      .sd-empty-btn-place:hover{background:#2A7A99;}
      .sd-empty-emoji{font-size:.95rem;}
      .sd-venue {
  ...
  document.addEventListener('error', function(e) {
    ...
    if (parent) {
      ...
      // Parent'ta zaten emoji span varsa goster
...
function renderVenuePage(venueId) {
  ...
  if (!document.getElementById('vp-styles')) {
    ...
    s.textContent = `
      ...
      .vp-contact-card{background:#fff;border-radius:16px;padding:16px 20px;display:flex;align-items:center;gap:12px;box-shadow:0 2px 10px rgba(26,39,68,.06);}
      .vp-contact-icon{flex-shrink:0;width:36px;height:36px;border-radius:10px;display:flex;align-items:center;justify-content:center;}
      .vp-contact-label{font-size:.6rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#A0AEC0;margin-bottom:2px;}
      ...
      .vp-contact-btn-outline{display:inline-flex;align-items:center;gap:4px;padding:8px 14px;border-radius:10px;border:1.5px solid rgba(26,39,68,.15);font-size:.78rem;font-weight:700;color:#1A2744;text-decoration:none;white-space:nowrap;transition:all .2s;}
      .vp-contact-btn-outline:hover{background:#1A2744;color:#fff;}
      .vp-wa-btn{display:inline-flex;align-items:center;gap:4px;padding:8px 14px;border-radius:10px;background:#25D366;color:#fff;font-size:.78rem;font-weight:700;text-decoration:none;white-space:nowrap;transition:all .2s;}
  ...
  document.getElementById('vp-body').innerHTML = `
    ...
    <div class="vp-body-wrap">
      <div class="vp-body-inner">
        ...
        ${isPremiumActive(v) && v.facilities && v.facilities.length > 0 ? '<div class="vp-section fade-up"><div class="vp-eyebrow">Olanaklar</div><h2 class="vp-stitle">Tesis Olanakları</h2><div style="display:flex;flex-wrap:wrap;gap:8px;">' + v.facilities.map(function(f) { var icons = {"WiFi":"📶","Otopark":"🅿️","Havuz":"🏊","Klima":"❄️","Asansör":"🛗","Engelli Erişimi":"♿","Kahvaltı Dahil":"☕","Restoran":"🍽","Akşam Yemeği":"🌙","Barbekü":"🔥","Deniz Manzaralı":"🌊","Deniz Manzarası":"🌊","Denize Sıfır":"🏖","Dağ Manzarası":"⛰","Bahçe":"🌿","Merkezi Konum":"📍","Odada TV":"📺","Odada Kasa":"🔒","Bebek Yatağı":"👶","Aile Odası":"🛏","Spa":"💆","Jakuzi":"🛁","Sauna":"🧖","Bisiklet":"🚲","Şömine":"🪵","Evcil Hayvan Kabul":"🐾","Evcil Hayvan":"🐾","Aile Dostu":"👨‍👩‍👧","+15 Yaş":"🔞","Sigara İçilmez":"🚭","Tarihi Yapı":"🏛","24 Saat Resepsiyon":"🕐","Transfer Hizmeti":"🚐"}; return '<span style="display:inline-flex;align-items:center;gap:5px;padding:7px 14px;border-radius:10px;background:rgba(26,39,68,.04);font-size:.78rem;font-weight:600;color:#4A5870;">' + (icons[f] || '✓') + ' ' + f + '</span>'; }).join('') + '</div></div>' : ''}
          ...
          <div class="vp-contact-grid${v.category === 'konaklama' ? ' vp-contact-grid-1col' : ''}">
            ...
            <div class="vp-contact-card vp-contact-wa">
              ...
              <div class="vp-contact-acts">
                ...
                <a href="${phWaUrl}" target="_blank" rel="noopener noreferrer" class="vp-wa-btn" onclick="if(window.trackAction)trackAction('${escAttr(v.id)}','whatsapp');">${waIconSmall} Yaz</a>
              </div>
            </div>` : `
        ...
        ${(() => {
          ...
          const tiles = imgs.slice(0, count).map((src, idx) =>
            '<div class="vp-gimg"><img src="' + (src.startsWith('http') ? src : base + src) + '" alt="' + v.title + ' — ' + (v.tagText || v.category || 'Mekan') + (v.location ? ', ' + v.location : '') + ' galeri fotoğrafı ' + (idx + 1) + '" loading="lazy" onload="this.classList.add(\'loaded\')"><div class="vp-gimg-overlay"></div></div>'
          ).join('');
          ...
          return '<div class="vp-section fade-up">' +
            ...
            '<div class="vp-ig-card">' +
              '<div class="vp-ig-left">' +
                ...
                '</div>' +
              '</div>' +
              '<a href="' + igUrl + '" target="_blank" rel="noopener noreferrer" class="vp-ig-btn" onclick="if(window.trackAction)trackAction(\'' + escAttr(v.id) + '\',\'instagram\')">Takip Et</a>' +
            '</div>' +
          '</div>';
        })()}
      ...
      <div class="vp-sticky-glass">
        <div class="vp-sticky-inner">
          ...
          <div class="vp-sticky-acts">
            ...
            ${waContactUrl && waPhone && waPhone.whatsapp && v.category !== 'konaklama' ? '<a href="' + waContactUrl + '" target="_blank" rel="noopener noreferrer" class="vp-sticky-btn vp-sticky-wa" onclick="if(window.trackAction)trackAction(\'' + escAttr(v.id) + '\',\'whatsapp\')"><svg width="14" height="14" viewBox="0 0 24 24" fill="#fff" style="vertical-align:middle;margin-right:3px"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.127.558 4.126 1.528 5.855L0 24l6.335-1.52C8.034 23.46 9.98 24 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.846 0-3.584-.479-5.104-1.32l-.369-.21-3.76.902.948-3.668-.223-.374A9.944 9.944 0 012 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/></svg>WhatsApp</a>' : ''}
            <a href="${mapsUrl}" target="_blank" rel="noopener noreferrer" class="vp-sticky-btn vp-sticky-map" onclick="if(window.trackAction)trackAction('${escAttr(v.id)}','directions')">🗺 Yol Tarifi Al</a>
          </div>
        </div>
      </div>
    </div>
  ...
  (function() {
    ...
    if (!dd) return;
    dd.innerHTML =
      '<a href="https://wa.me/?text=' + encodeURIComponent(waText) + '" target="_blank" rel="noopener noreferrer" class="vp-share-opt">' +
      ...
      '<a href="https://www.facebook.com/sharer/sharer.php?u=' + encodeURIComponent(url) + '" target="_blank" rel="noopener noreferrer" class="vp-share-opt">' +
      ...
      '<a href="https://www.instagram.com/" target="_blank" rel="noopener noreferrer" class="vp-share-opt">' +
      ...
      '<a href="https://x.com/intent/tweet?text=' + encodeURIComponent(text) + '&url=' + encodeURIComponent(url) + '" target="_blank" rel="noopener noreferrer" class="vp-share-opt">' +
...
function renderVillagePage(villageId) {
  ...
  // Reel like/bookmark state restore (varsa)
  try { if (window.akReelRestoreState) akReelRestoreState(); } catch(e) {}
  ...
  if (dd) {
    dd.innerHTML =
      '<a href="https://wa.me/?text=' + encodeURIComponent(vgWaText) + '" target="_blank" rel="noopener noreferrer" class="vp-share-opt">' +
        '<span class="vp-share-opt-icon" style="background:#25D366"><svg width="16" height="16" viewBox="0 0 24 24" fill="#fff"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.127.558 4.126 1.528 5.855L0 24l6.335-1.52C8.034 23.46 9.98 24 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.846 0-3.584-.479-5.104-1.32l-.369-.21-3.76.902.948-3.668-.223-.374A9.944 9.944 0 012 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/></svg></span>' +
        '<span>WhatsApp</span></a>' +
      '<a href="https://www.facebook.com/sharer/sharer.php?u=' + encodeURIComponent(vgUrl) + '" target="_blank" rel="noopener noreferrer" class="vp-share-opt">' +
        '<span class="vp-share-opt-icon" style="background:#1877F2"><svg width="14" height="14" viewBox="0 0 24 24" fill="#fff"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg></span>' +
        '<span>Facebook</span></a>' +
      '<a href="https://x.com/intent/tweet?text=' + encodeURIComponent(vgText) + '&url=' + encodeURIComponent(vgUrl) + '" target="_blank" rel="noopener noreferrer" class="vp-share-opt">' +
        '<span class="vp-share-opt-icon" style="background:#000"><svg width="12" height="12" viewBox="0 0 24 24" fill="#fff"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg></span>' +
        '<span>X</span></a>' +
...
function renderPlacePage(placeId) {
  ...
  if (nearbyVenues.length > 0) {
    ...
    nearbyVenues.forEach(function(venue) {
      ...
      bodyHtml += '<div style="padding:14px 16px 16px;"><div style="display:flex;align-items:center;justify-content:space-between;"><h4 style="font-family:\'Plus Jakarta Sans\',sans-serif;font-weight:700;font-size:.88rem;color:var(--navy);margin:0;">' + venue.title + '</h4>' + (dTxt2 ? '<span style="font-size:.62rem;font-weight:700;color:var(--terra);white-space:nowrap;">~' + dTxt2 + '</span>' : '') + '</div></div></a>';
    });
    bodyHtml += '</div></div>';
  }
...
(function() {
  ...
  chatCSS.textContent = `
    .ai-chat-fab{position:fixed;bottom:24px;right:24px;z-index:400;width:58px;height:58px;border-radius:50%;background:linear-gradient(135deg,#C4521A 0%,#A3431A 100%);color:#fff;border:2px solid rgba(255,255,255,.25);cursor:pointer;box-shadow:0 8px 24px rgba(196,82,26,.35),0 0 0 0 rgba(212,147,90,.5);display:flex;align-items:center;justify-content:center;font-size:1.35rem;transition:all .3s cubic-bezier(.16,1,.3,1);animation:aiFabPulse 2.6s ease-in-out infinite;}
    @keyframes aiFabPulse{0%,100%{box-shadow:0 8px 24px rgba(196,82,26,.35),0 0 0 0 rgba(212,147,90,.5)}50%{box-shadow:0 8px 24px rgba(196,82,26,.4),0 0 0 12px rgba(212,147,90,0)}}
    .ai-chat-fab:hover{transform:scale(1.08);box-shadow:0 10px 32px rgba(196,82,26,.45);}
    .ai-chat-fab.open{animation:none;transform:rotate(0) scale(1);font-size:1.1rem;}
    .ai-chat-panel{position:fixed;bottom:96px;right:24px;z-index:399;width:380px;max-width:calc(100vw - 32px);max-height:540px;background:var(--cream-light,#FAF7F2);border:1px solid rgba(26,39,68,.08);border-radius:24px;box-shadow:0 24px 72px rgba(26,39,68,.22),0 0 0 1px rgba(212,147,90,.06);display:none;flex-direction:column;overflow:hidden;backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);}
    .ai-chat-panel.open{display:flex;animation:aiChatIn .4s cubic-bezier(.22,1,.36,1);}
    @keyframes aiChatIn{from{opacity:0;transform:translateY(12px) scale(.94)}to{opacity:1;transform:translateY(0) scale(1)}}
    .ai-chat-head{padding:16px 20px;background:linear-gradient(135deg,#1A2744 0%,#243255 100%);color:#F5EDE0;display:flex;align-items:center;gap:12px;position:relative;overflow:hidden;border-bottom:1px solid rgba(212,147,90,.15);}
    .ai-chat-head::before{content:'';position:absolute;width:140px;height:140px;border-radius:50%;background:radial-gradient(circle,rgba(212,147,90,.18) 0%,transparent 70%);top:-50px;right:-30px;pointer-events:none;}
    .ai-chat-avatar{width:38px;height:38px;border-radius:12px;background:linear-gradient(135deg,rgba(212,147,90,.25),rgba(196,82,26,.18));border:1.5px solid rgba(212,147,90,.35);display:flex;align-items:center;justify-content:center;font-size:1.1rem;flex-shrink:0;position:relative;}
    .ai-chat-avatar::after{content:'';position:absolute;bottom:-2px;right:-2px;width:11px;height:11px;background:#22c55e;border-radius:50%;border:2px solid #1A2744;box-shadow:0 0 8px rgba(34,197,94,.6);animation:aiOnlinePulse 2s ease-in-out infinite;}
    @keyframes aiOnlinePulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.75;transform:scale(.92)}}
    .ai-chat-head-info{flex:1;min-width:0;}
    .ai-chat-title{font-family:'Plus Jakarta Sans',sans-serif;font-weight:800;font-size:.92rem;letter-spacing:-.01em;display:flex;align-items:center;gap:8px;flex-wrap:wrap;}
    .ai-chat-badge{display:inline-flex;align-items:center;padding:2px 8px;background:linear-gradient(135deg,rgba(212,147,90,.22),rgba(196,82,26,.16));border:1px solid rgba(212,147,90,.35);border-radius:8px;font-size:.58rem;font-weight:700;color:#D4935A;letter-spacing:.05em;text-transform:uppercase;}
    .ai-chat-sub{font-size:.64rem;color:rgba(245,237,224,.45);margin-top:2px;font-weight:500;}
    ...
    .ai-msg.bot{background:#fff;color:#1A2744;border:1px solid rgba(26,39,68,.06);align-self:flex-start;border-bottom-left-radius:4px;box-shadow:0 1px 3px rgba(26,39,68,.04);}
    .ai-msg.user{background:linear-gradient(135deg,#1A2744,#243255);color:#F5EDE0;align-self:flex-end;border-bottom-right-radius:4px;box-shadow:0 2px 8px rgba(26,39,68,.15);}
    ...
    .ai-chat-input{display:flex;gap:8px;padding:12px 16px 10px;border-top:1px solid rgba(26,39,68,.06);background:var(--cream-light,#FAF7F2);}
    .ai-chat-input input{flex:1;padding:11px 14px;border:1.5px solid rgba(26,39,68,.1);border-radius:12px;font-size:16px;font-family:inherit;outline:none;background:#fff;transition:border-color .2s,box-shadow .2s;}
    .ai-chat-input input:focus{border-color:var(--terra,#C4521A);box-shadow:0 0 0 3px rgba(196,82,26,.1);}
    .ai-chat-input button{width:40px;height:40px;border-radius:12px;background:linear-gradient(135deg,#C4521A 0%,#A3431A 100%);color:#fff;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:1rem;flex-shrink:0;transition:transform .2s,box-shadow .2s;box-shadow:0 2px 8px rgba(196,82,26,.25);}
    .ai-chat-input button:hover:not(:disabled){transform:translateY(-1px);box-shadow:0 6px 16px rgba(196,82,26,.4);}
    .ai-chat-input button:disabled{opacity:.4;cursor:default;box-shadow:none;}
    .ai-chat-foot{display:flex;align-items:center;justify-content:space-between;padding:0 16px 10px;gap:12px;background:var(--cream-light,#FAF7F2);}
    .ai-chat-limit{font-size:.62rem;color:rgba(26,39,68,.5);font-weight:600;letter-spacing:.01em;display:flex;align-items:center;gap:6px;}
    .ai-chat-limit::before{content:'';width:6px;height:6px;border-radius:50%;background:#22c55e;box-shadow:0 0 4px rgba(34,197,94,.5);flex-shrink:0;}
    .ai-chat-clear{background:none;border:none;font-size:.62rem;color:rgba(26,39,68,.4);cursor:pointer;font-family:inherit;padding:2px 0;transition:color .2s;}
    .ai-chat-clear:hover{color:var(--terra,#C4521A);}
    ...
    .ai-chat-close{display:none;width:32px;height:32px;border-radius:50%;background:rgba(245,237,224,.12);border:none;color:rgba(245,237,224,.7);font-size:.9rem;cursor:pointer;align-items:center;justify-content:center;flex-shrink:0;transition:background .2s;}
    .ai-chat-close:hover{background:rgba(245,237,224,.2);}
    @media(max-width:480px){.ai-chat-panel{bottom:76px;right:20px;left:20px;width:auto;max-width:100%;max-height:70vh;border-radius:20px;}.ai-chat-fab{bottom:16px;right:16px;width:50px;height:50px;font-size:1.2rem;}.ai-chat-fab.open{animation:none;font-size:1rem;}.ai-chat-close{display:none;}}
  `;
  ...
  var chatHTML = `
    ...
    <div class="ai-chat-panel" id="ai-chat-panel">
      ...
      <div class="ai-chat-foot">
        <span class="ai-chat-limit" id="ai-chat-limit"></span>
        <button class="ai-chat-clear" onclick="aiClearHistory()">Sohbeti Temizle</button>
      </div>
    </div>
  `;
  ...
  window.aiChatSend = async function() {
    ...
    if (state.count >= getAiDailyLimit()) {
      ...
      igBtn.innerHTML = '<a href="https://www.instagram.com/assosukesfet/" target="_blank" rel="noopener noreferrer" style="display:inline-flex;align-items:center;gap:8px;padding:8px 16px;border-radius:12px;background:linear-gradient(135deg,#833AB4,#E1306C,#F77737);color:#fff;font-size:.78rem;font-weight:700;text-decoration:none;transition:opacity .2s;" onmouseover="this.style.opacity=0.85" onmouseout="this.style.opacity=1">📸 Takip Et</a>';
    ...
    try {
      ...
      if (data.reply) {
        ...
        if (data.limitReached) {
          ...
          igBtn2.innerHTML = '<a href="https://www.instagram.com/assosukesfet/" target="_blank" rel="noopener noreferrer" style="display:inline-flex;align-items:center;gap:8px;padding:8px 16px;border-radius:12px;background:linear-gradient(135deg,#833AB4,#E1306C,#F77737);color:#fff;font-size:.78rem;font-weight:700;text-decoration:none;">📸 Takip Et</a>';
  ...
  function mdToHtml(text) {
    ...
    var result = text.replace(/\[([^\]]+)\]\((https?:\/\/)?assosukesfet\.com\/([^)]+)\)/g, function(match, label, proto, path) {
      ...
      return '<a href="https://assosukesfet.com/' + path + '" target="_blank" rel="noopener noreferrer" style="' + linkStyle + '">' + label + '</a>';
    });
    ...
    result = result.replace(/(?<!["=\/])((?:https?:\/\/)?assosukesfet\.com\/[^\s<,)"]+)/g, function(url) {
      ...
      return '<a href="' + clean + '" target="_blank" rel="noopener noreferrer" style="' + linkStyle + '">' + name + '</a>';
    });
