  ...
  document.getElementById('vp-body').innerHTML = `
    ...
    <div class="vp-body-wrap">
      <div class="vp-body-inner">
        ...
        ${isPremiumActive(v) && v.rooms && v.rooms.length > 0 ? '<div class="vp-section fade-up"><div class="vp-eyebrow">Konaklama</div><h2 class="vp-stitle">Oda Tipleri</h2><div style="display:grid;gap:12px;">' + '<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:14px;">' + v.rooms.map(function(r) { var viewIcons = {"Deniz Manzarası":"🌊","Kara Manzarası":"🏔","Bahçe Manzarası":"🌿","Havuz Manzarası":"🏊"}; return '<div style="background:#fff;border:1px solid rgba(26,39,68,.07);border-radius:16px;overflow:hidden;transition:all .25s;" onmouseover="this.style.boxShadow=\'0 8px 28px rgba(26,39,68,.08)\';this.style.transform=\'translateY(-3px)\'" onmouseout="this.style.boxShadow=\'none\';this.style.transform=\'\'">' + (r.image ? '<div style="aspect-ratio:16/10;overflow:hidden;background:linear-gradient(90deg,#e8e4de 25%,#f0ece6 50%,#e8e4de 75%);background-size:800px 100%;"><img src="' + r.image + '" style="width:100%;height:100%;object-fit:cover;opacity:0;transition:opacity .6s ease;" loading="lazy" onload="this.style.opacity=1" alt="' + r.name + '"></div>' : '<div style="aspect-ratio:16/10;background:linear-gradient(135deg,rgba(90,122,86,.08),rgba(90,122,86,.03));display:flex;align-items:center;justify-content:center;font-size:2.2rem;">🛏</div>') + '<div style="padding:14px 16px 16px;"><div style="font-weight:700;font-size:.9rem;color:#1A2744;margin-bottom:6px;">' + r.name + '</div><div style="display:flex;flex-wrap:wrap;gap:6px;">' + [r.capacity ? '<span style="font-size:.65rem;font-weight:600;padding:3px 8px;border-radius:6px;background:rgba(26,39,68,.05);color:#4A5870;">👤 ' + r.capacity + ' Kişi</span>' : '', r.bed ? '<span style="font-size:.65rem;font-weight:600;padding:3px 8px;border-radius:6px;background:rgba(26,39,68,.05);color:#4A5870;">🛏 ' + r.bed + '</span>' : '', r.size ? '<span style="font-size:.65rem;font-weight:600;padding:3px 8px;border-radius:6px;background:rgba(26,39,68,.05);color:#4A5870;">📐 ' + r.size + ' m²</span>' : '', r.view ? '<span style="font-size:.65rem;font-weight:600;padding:3px 8px;border-radius:6px;background:rgba(26,107,138,.08);color:#1A6B8A;">' + (viewIcons[r.view] || '🏞') + ' ' + r.view + '</span>' : ''].filter(Boolean).join('') + '</div></div></div>'; }).join('') + '</div>' + '</div></div>' : ''}
