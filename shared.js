...
function renderVenuePage(venueId) {
  ...
  document.getElementById('vp-hero').innerHTML = `
    <div class="vp-hero">
      ...
      ${v.images && v.images.length > 0 ? '<div style="position:absolute;inset:0;z-index:0;background-image:url(' + (v.images[0].startsWith('http') ? v.images[0] : base + v.images[0]) + ');background-size:cover;background-position:center;opacity:0;transition:opacity 1s ease;" onload="this.style.opacity=\'0.25\'" id="vp-hero-img-bg"></div><script>var hImg=new Image();hImg.onload=function(){var el=document.getElementById("vp-hero-img-bg");if(el)el.style.opacity="0.25";};hImg.src="' + (v.images[0].startsWith('http') ? v.images[0] : base + v.images[0]) + '";<\/script>' : ''}
