  ...
  document.getElementById('vp-body').innerHTML = `
    ...
    <div class="vp-body-wrap">
      <div class="vp-body-inner">
        ...
        ${isPremiumActive(v) && v.rooms && v.rooms.length > 0 ? '<div class="vp-section fade-up"><div class="vp-eyebrow">Konaklama</div><h2 class="vp-stitle">Oda Tipleri</h2><div style="display:grid;gap:12px;">' + v.rooms.map(function(r) { return '<div style="display:flex;align-items:center;gap:14px;padding:' + (r.image ? '0' : '14px 18px') + ';background:rgba(90,122,86,.04);border:1px solid rgba(90,122,86,.1);border-radius:14px;overflow:hidden;">' + (r.image ? '<img src="' + r.image + '" style="width:90px;height:70px;object-fit:cover;flex-shrink:0" alt="' + r.name + '">' : '<span style="font-size:1.4rem;padding-left:4px;">🛏</span>') + '<div style="flex:1;padding:' + (r.image ? '10px 14px 10px 0' : '0') + '"><div style="font-weight:700;font-size:.88rem;color:#1A2744;">' + r.name + '</div><div style="font-size:.72rem;color:#718096;margin-top:2px;">' + [r.capacity ? r.capacity + ' Kişi' : '', r.bed || '', r.size ? r.size + ' m²' : '', r.view || ''].filter(Boolean).join(' · ') + '</div></div></div>'; }).join('') + '</div></div>' : ''}
